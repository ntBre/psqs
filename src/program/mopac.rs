...
pub struct Mopac {
    ...
    /// the file in which to store the parameters. set to None on creation, and
    /// only created when `write_params` is called
...
impl Mopac {
    ...
    /// files yet, except for the parameters
    ...
    pub fn build_jobs(
        ...
        for mol in moles {
            ...
            job_num += 1;
            let mut mop =
                Mopac::new(filename, tmpl.clone(), charge, mol.clone());
            if let Some(params) = params.clone() {
                mop.write_params(&params, dir);
            }
            let mut job = Job::new(mop, count);
            job.coeff = coeff;
            jobs.push(job);
            count += 1;
        }
    ...
    /// write `params` to a filename generated by hashing `self.filename` in the
    /// directory specified by `dir`
    fn write_params(&mut self, params: &Params, dir: &str) {
        let body = params.to_string();
        let mut s = DefaultHasher::new();
        self.filename.hash(&mut s);
        let param_file = format!("{}/{}", dir, s.finish());
        {
            use std::fmt::Write;
            write!(self.template.header, " external={}", param_file).unwrap();
        }
        let mut file = match File::create(&param_file) {
            Ok(f) => f,
            Err(e) => {
                eprintln!("failed to create {} with {}", &param_file, e);
                std::process::exit(1);
            }
        };
